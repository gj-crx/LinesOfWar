...
namespace GameCore
{
    public class GameController
    {
        ...
        {
            EconomyThread = new Thread(ControlEconomy);

            EconomyThread.Start();
        }


        private void RecalculateAllTradeRoutes()
        {
            foreach (var province in GameManager.dataBase.AllPaths)
            {
        ...
        public void ControlEconomy()
        {
            ...
            foreach (var province in GameManager.dataBase.AllPaths) province.province.Produce();
            foreach (var province in GameManager.dataBase.AllPaths) province.province.Consume();
            foreach (var province in GameManager.dataBase.AllPaths) province.province.Transport();
